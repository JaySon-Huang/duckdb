...
//! Resolve the column strlen(VARCHAR) or octet_length(BLOB) is applied to. length()/char_length()
//! count code points and are deliberately not recognized here - the byte length statistics do not
//! answer them. octet_length(BIT) is excluded as well: BIT's stored size carries a padding byte.
...
//! Resolve MIN/MAX(f(col)) from the per-argument monotonicity annotation that also drives
//! MonotonePreimageRule and PropagateMonotoneBounds. Exactly one argument may hold the column, so
//! that the mapped endpoint is a value some row actually attains - with two column arguments
//! f(min, min) need not occur in the data at all. Every other argument has to be a constant.
...
//! statistics - and those length fields stay exact when the min/max *value* statistics are truncated
//! to a prefix, which is why this folds partitions the plain string MIN/MAX can only vote on.
struct ByteLengthFoldClient {
	...
	FoldPartitionState ClassifyPartition(const FoldPartition &partition, Value &value) const {
		...
		if (partition.filter_result != FilterPropagateResult::FILTER_ALWAYS_TRUE) {
			// the filter cuts the partition: the surviving rows are a subset, so the length is a
			// bound over them rather than the value they attain
	...
	bool ExcludesCandidate(const Value &bound, const Value &candidate) const {
		// A length is an exact integer even when the string that produced it was truncated, so unlike
		// the string min/max the bound is always safe to compare. MIN reads the shortest string of the
		// partition, so its bound is a lower one; MAX reads the longest, so its bound is an upper one.
...
//! MIN/MAX over a monotone function of a column. Monotonicity maps the column's minimum to the
//! minimum of the mapped values and its maximum to their maximum (the other way round for a
//! descending function), and both endpoints are attained by a row, so the aggregate is answered from
//! the column statistics alone. Only exact partitions fold: a bound endpoint is not a value any
//! surviving row holds, and mapping it would assume the function is total on inputs that never occur.
struct MonotoneFoldClient {
	...
	FoldPartitionState ClassifyPartition(const FoldPartition &partition, Value &value) const {
		...
		// an ascending function attains MIN at the column minimum and MAX at the column maximum, a
		// descending one the other way round
		const bool use_min_endpoint = info.is_min != info.decreasing;
		auto &func = info.function->Cast<BoundFunctionExpression>();
		auto args = info.arg_values;
		args[info.column_arg] = use_min_endpoint ? NumericStats::Min(*column_stats) : NumericStats::Max(*column_stats);
		Value mapped;
		if (!StatisticsPropagator::TryEvaluateAtConstants(context, func, args, mapped)) {
			return FoldPartitionState::NO_INFO;
		}
		if (mapped.IsNull()) {
			return FoldPartitionState::NO_INFO;
		}
		// NaN is ordered above every other value, so the mapping is not monotone at that point
		if ((mapped.type().id() == LogicalTypeId::DOUBLE && Value::IsNan(mapped.GetValue<double>())) ||
		    (mapped.type().id() == LogicalTypeId::FLOAT && Value::IsNan(mapped.GetValue<float>()))) {
			return FoldPartitionState::NO_INFO;
		}
	...
	bool ExcludesCandidate(const Value &, const Value &) const {
		// no bound is ever mapped, so there is nothing to exclude with
		return false;
	}
...
void StatisticsPropagator::TryExecuteAggregates(LogicalAggregate &aggr, unique_ptr<LogicalOperator> &node_ptr) {
	...
	while (child_ref.get().type == LogicalOperatorType::LOGICAL_PROJECTION) {
		...
		// byte-length entries chase bare column references one level down; a projection computing the
		// byte length from a column is what the aggregate consumes directly
		...
		// a monotone entry only chases bare column references: if this projection computes its
		// argument, the argument is no longer a column the statistics describe
